  ...
  return <div className="min-h-screen bg-background text-on-surface">
    ...
    <header className="teacher-top flex justify-between items-center w-[calc(100%-16rem)] ml-64 px-lg h-16 sticky top-0 bg-surface border-b border-outline-variant z-40 backdrop-blur-md bg-white/95">
      <div className="flex items-center gap-md flex-1 min-w-0">
        <button onClick={() => setPage('dashboard')} className="p-xs hover:bg-surface-container-high rounded-full transition-colors"><Icon className="text-primary">arrow_back</Icon></button>
      ...
      <div className="flex items-center gap-md">
        ...
        <div className="h-8 w-8 rounded-full overflow-hidden border border-outline-variant bg-primary-container text-white grid place-items-center text-sm font-bold">PV</div>
      </div>
    </header>
    <main className="teacher-main ml-64 min-h-screen p-lg max-w-[1500px]">{children}</main>
  </div>
}
